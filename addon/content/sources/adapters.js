...
//   { ast, limit, yearFrom, yearTo, oaOnly, fulltextOnly, key, secret(id), email, onPage }
// and resolves to { records, query, total, capped?, note? }. `query` is the exact string
// sent, which goes into the search protocol so runs are reproducible.
//
// limit 0 means no limit: every result the database returns for the query, page by page,
// up to what its API hands out (e.g. Scopus 5,000); `capped` then says where it stopped.
...
(() => {
  ...
  /**
   * Page through a search: until the limit (0 = none), the last result, the API's maximum,
   * or a stop. fetch({offset, cursor, n, page}) → {records, total?, next?}
   * `next` is the cursor for cursor paging (null / missing at the end); offset paging
   * leaves it undefined. An error on a later page keeps what came before.
   * paged: the API takes page numbers of a fixed size (DOAJ, Zenodo, …); fixed: a cursor
   * API whose pages have a fixed size whatever is asked (Semantic Scholar). Both can stop
   * inside a page; the position then says how much of it was kept.
   * o.resume {offset, cursor?, skip?} continues where an earlier search stopped; the
   * result's `pos` is where this one stopped (null when the database has nothing more).
   */
  async function collect(o, { size, max = 10000, fetch, paged = false, fixed = false }) {
    const want = o.limit > 0 ? o.limit : Infinity;
    const records = [];
    let total = 0;
    let offset = o.resume?.offset || 0;
    let cursor = o.resume?.cursor;
    let skip = o.resume?.skip || 0;
    ...
    let cut = false;
    for (let i = 0; ; i++) {
      if (i && ZR.Activity?.stopping) {
        note = "stopped";
        break;
      }
      const n = Math.min(size, want - records.length, max - offset);
      if (n <= 0) break;
      const page = Math.floor(offset / size);
      const drop = paged ? offset - page * size : fixed ? skip : 0;
      pageCursor = cursor;
      let r;
      try {
        r = await fetch({ offset, cursor, n, page });
      } catch (e) {
        if (!i) throw e;
        error = e;
        note = `stopped after ${offset}: ${e.message}`;
        break;
      }
      total = r.total ?? total;
      const fresh = r.records.slice(drop);
      const got = fresh.slice(0, n);
      records.push(...got);
      offset += got.length;
      o.onPage?.(Math.min(records.length, want), total);
      if (fresh.length > got.length) {
        // stopped inside this page (the limit): the rest of it is fetched next time
        if (fixed) skip = drop + got.length;
        cut = true;
        break;
      }
      skip = 0;
      if (!r.records.length || !fresh.length || (total && offset >= total)) {
        ended = true;
        break;
      }
      if (r.next !== undefined) {
        if (!r.next || r.next === cursor) {
          ended = true;
          break;
        }
        cursor = r.next;
      } else if (r.records.length < (paged ? size : n)) {
        ended = true;
        break;
      }
    }
    const capped = !ended && offset >= max && total > offset ? max : 0;
    let pos = null;
    if (!ended && !capped) pos = fixed && cut ? { offset, ...(pageCursor ? { cursor: pageCursor } : {}), skip } : { offset, ...(cursor ? { cursor } : {}) };
    return { records: records.slice(0, want), total, capped, note, error, pos };
  }
  const out = (c, query) => ({ records: c.records, query, total: c.total, capped: c.capped, note: c.note, error: c.error, pos: c.pos });
  ...
  attach("openalex", async (o) => {
    ...
    const c = await collect(o, {
      size: 200,
      max: 1e6,
      fetch: async ({ cursor, n }) => {
        const url = "https://api.openalex.org/works?" + U.qs({ search: query, filter: filters.join(","), per_page: n, cursor: cursor || "*", select, api_key: o.key, mailto: o.key ? "" : o.email });
        const data = await U.getJSON(url, { retryAfterMax: 45000 });
        return { records: (data.results || []).map(mapOpenAlex), total: data.meta?.count ?? 0, next: data.meta?.next_cursor || null };
      },
    });
    return out(c, query);
  });
  ...
  attach("crossref", async (o) => {
    ...
    const per = o.limit > 0 ? Math.max(5, Math.ceil(o.limit / queries.length)) : 0;
    ...
    for (const [k, q] of queries.entries()) {
      const from = o.resume?.branches ? o.resume.branches[k] : null;
      if (o.resume?.branches && !from) {
        branches.push(null); // this branch was complete
        continue;
      }
      const c = await collect(Object.assign({}, o, { limit: per, resume: from ? { offset: from.offset } : null }), {
        size: 500,
        max: 1e6,
        fetch: async ({ cursor, offset, n }) => {
          await ZR.Sources.throttle("crossref", o.email ? 400 : 1100);
          const byOffset = !cursor && offset > 0; // continuing: Crossref takes an offset up to 10,000
          const data = await U.getJSON("https://api.crossref.org/works?" + U.qs(Object.assign({ "query.bibliographic": q, rows: n, filter: filters.join(","), select, mailto: o.email }, byOffset ? { offset } : { cursor: cursor || "*" })));
          return { records: (data.message?.items || []).map(mapCrossref), total: data.message?.["total-results"] || 0, next: byOffset ? undefined : data.message?.["next-cursor"] || null };
        },
      });
      records.push(...c.records);
      total += c.total;
      capped = Math.max(capped, c.capped);
      note = note || c.note;
      error = error || c.error;
      branches.push(c.pos ? { offset: c.pos.offset } : null);
    }
    return { records, query: queries.map((q) => `[${q}]`).join(" ∪ "), total, capped, note, error, pos: branches.some(Boolean) ? { branches } : null };
  });
  ...
  attach("semanticscholar", async (o) => {
    ...
    const c = await collect(o, {
      ...
      fetch: async ({ cursor }) => {
        ...
        const data = await U.getJSON(url + (cursor ? "&token=" + encodeURIComponent(cursor) : ""), { headers, retryAfterMax: 20000 });
        return { records: (data.data || []).map(mapS2), total: data.total || 0, next: data.token || null };
      },
    });
    return out(c, query + " (sorted by citations)");
  });
  ...
  attach("arxiv", async (o) => {
    ...
    const c = await collect(o, {
      size: 500,
      max: 30000,
      fetch: async ({ offset, n }) => {
        await ZR.Sources.throttle("arxiv", 3100);
        const doc = parseXML(await U.getText("https://export.arxiv.org/api/query?" + U.qs({ search_query: query, start: offset, max_results: n, sortBy: "relevance" })));
        const entries = Array.from(doc.getElementsByTagName("entry")).filter((e) => e.getElementsByTagName("title")[0]?.textContent !== "Error");
        return { records: entries.map(mapArxivEntry), total: parseInt(doc.getElementsByTagNameNS("http://a9.com/-/spec/opensearch/1.1/", "totalResults")[0]?.textContent || "0", 10) };
      },
    });
    return out(c, query);
  });
  ...
  attach("europepmc", async (o) => {
    ...
    const c = await collect(o, {
      ...
      max: 1e6,
      fetch: async ({ cursor, n }) => {
        const data = await U.getJSON("https://www.ebi.ac.uk/europepmc/webservices/rest/search?" + U.qs({ query, resultType: "core", format: "json", pageSize: n, cursorMark: cursor || "*" }));
        return { records: (data.resultList?.result || []).map(mapEPMC), total: data.hitCount || 0, next: data.nextCursorMark || null };
      },
    });
    return out(c, query);
  });
  ...
  attach("pubmed", async (o) => {
    ...
    const c = await collect(o, {
      size: 200,
      max: 9999, // esearch hands out the first 10,000
      fetch: async ({ offset, n }) => {
        await ZR.Sources.throttle("pubmed", gap);
        const search = await U.getJSON(base + "esearch.fcgi?" + U.qs({ db: "pubmed", term, retmode: "json", retstart: offset, retmax: n, ...common }));
        const ids = search.esearchresult?.idlist || [];
        return { records: ids.length ? await pubmedRecords(ids) : [], total: parseInt(search.esearchresult?.count || "0", 10) };
      },
    });
    return out(c, term);

    async function pubmedRecords(ids) {
    await ZR.Sources.throttle("pubmed", gap);
  ...
  attach("doaj", async (o) => {
    ...
    const c = await collect(o, {
      ...
      fetch: async ({ page }) => {
        ...
        const data = await U.getJSON(`https://doaj.org/api/search/articles/${encodeURIComponent(query)}?` + U.qs({ page: page + 1, pageSize: 100 }));
        return { records: doajRecords(data), total: data.total || 0 };
      },
    });
    return out(c, query);
  });
  function doajRecords(data) {
  ...
  attach("hal", async (o) => {
    ...
    const c = await collect(o, {
      ...
      max: 10000,
      fetch: async ({ offset, n }) => {
        let url = "https://api.archives-ouvertes.fr/search/?" + U.qs({ q, rows: n, start: offset, wt: "json", fl });
        for (const f of fq) url += "&fq=" + encodeURIComponent(f);
        const data = await U.getJSON(url);
        return { records: (data.response?.docs || []).map(halRecord), total: data.response?.numFound || 0 };
      },
    });
    return out(c, q + (fq.length ? ` | fq: ${fq.join("; ")}` : ""));
  });
  const halRecord = (d) =>
      R.make("hal", {
        ids: { hal: d.halId_s },
  ...
  attach("zenodo", async (o) => {
    ...
    const c = await collect(o, {
      ...
      paged: true,
      fetch: async ({ page }) => {
        const data = await U.getJSON("https://zenodo.org/api/records?" + U.qs({ q: query, size: 25, page: page + 1, type: "publication", sort: "bestmatch" }), { headers: { Accept: "application/json" } });
        let records = zenodoRecords(data);
        if (o.oaOnly || o.fulltextOnly) records = records.filter((r) => r.pdfURLs.length);
        return { records, total: data.hits?.total ?? 0 };
      },
    });
    return out(c, query);
  });
  function zenodoRecords(data) {
  ...
  attach("osti", async (o) => {
    ...
    const c = await collect(o, {
      ...
      paged: true,
      fetch: async ({ page }) => {
        const data = await U.getJSON("https://www.osti.gov/api/v1/records?" + U.qs(Object.assign({ rows: 100, page: page + 1 }, params)), { headers: { Accept: "application/json" } });
        return { records: ostiRecords(Array.isArray(data) ? data : data.records || []) };
      },
    });
    return out(c, q);
  });
  function ostiRecords(list) {
  ...
  attach("dblp", async (o) => {
    ...
    for (const [k, q] of queries.entries()) {
      ...
      const c = await collect(Object.assign({}, o, { resume: from }), {
        size: 1000,
        max: 10000,
        fetch: async ({ offset, n }) => {
          await ZR.Sources.throttle("dblp", 1000);
          const res = await ZR.http("GET", "https://dblp.org/search/publ/api?" + U.qs({ q, format: "json", h: n, f: offset }), {});
          const data = assertJSONResponse(res, "dblp");
          const hits = data.result?.hits?.hit || [];
          return { records: hits.map((h) => dblpRecord(h, o)), total: parseInt(data.result?.hits?.["@total"] || "0", 10) };
        },
      });
      records.push(...c.records.filter(Boolean));
      total += c.total;
      note = note || c.note;
      error = error || c.error;
      branches.push(c.pos);
    }
    return { records, query: queries.map((q) => `[${q}]`).join(" ∪ "), total, note, error, pos: branches.some(Boolean) ? { branches } : null };
  });
  function dblpRecord(h, o) {
    {
        const i = h.info || {};
        let authors = i.authors?.author || [];
        if (!Array.isArray(authors)) authors = [authors];
        const year = parseInt(i.year, 10);
        if ((o.yearFrom && year < o.yearFrom) || (o.yearTo && year > o.yearTo)) return null;
        return (
          R.make("dblp", {
            title: i.title,
  ...
  attach("core", async (o) => {
    ...
    const c = await collect(o, {
      ...
      fetch: async ({ offset, n }) => {
        ...
        const data = await U.getJSON("https://api.core.ac.uk/v3/search/works/?" + U.qs({ q, limit: n, offset, exclude: "fullText" }), { headers });
        let records = (data.results || []).map(coreRecord);
        if (o.oaOnly || o.fulltextOnly) records = records.filter((r) => r.pdfURLs.length);
        return { records, total: data.totalHits || 0 };
      },
    });
    return out(c, q);
  });
  ...
  attach("springer", async (o) => {
    ...
    const c = await collect(o, {
      ...
      fetch: async ({ offset, n }) => {
        ...
        const data = await U.getJSON("https://api.springernature.com/meta/v2/json?" + U.qs({ q, api_key: o.key, s: offset + 1, p: n }));
        return { records: (data.records || []).map(springerRecord), total: parseInt(data.result?.[0]?.total || "0", 10) };
      },
    });
    return out(c, q);
  });
  function springerRecord(r) {
    {
  ...
  attach("ieee", async (o) => {
    ...
    const params = { apikey: o.key, querytext, start_year: o.yearFrom, end_year: o.yearTo, format: "json" };
    if (o.oaOnly || o.fulltextOnly) params.open_access = "True";
    const c = await collect(o, {
      size: 200,
      max: 10000,
      fetch: async ({ offset, n }) => {
        const data = await U.getJSON("https://ieeexploreapi.ieee.org/api/v1/search/articles?" + U.qs(Object.assign({ max_records: n, start_record: offset + 1 }, params)));
        return { records: (data.articles || []).map(ieeeRecord), total: data.total_records || 0 };
      },
    });
    return out(c, querytext);
  });
  const ieeeRecord = (a) =>
      R.make("ieee", {
        title: a.title,
  ...
  attach("wos", async (o) => {
    ...
    const c = await collect(o, {
      ...
      fetch: async ({ page }) => {
        ...
        const data = await U.getJSON("https://api.clarivate.com/apis/wos-starter/v1/documents?" + U.qs({ db: "WOS", q, limit: 50, page: page + 1 }), { headers: { "X-ApiKey": o.key } });
        return { records: (data.hits || []).map(wosRecord), total: data.metadata?.total || 0 };
      },
    });
    return out(c, q);
  });
  function wosRecord(h) {
    {
      const src = h.source || {};
  ...
  attach("scopus", async (o) => {
    ...
    let view = "COMPLETE";
    const fetchView = (v, start, n) => U.getJSON("https://api.elsevier.com/content/search/scopus?" + U.qs({ query, count: Math.min(n, v === "COMPLETE" ? 25 : 200), start, view: v }), { headers: elsevierHeaders(o), noRetry: true });
    const c = await collect(o, {
      size: 200,
      max: 5000, // the Scopus Search API hands out the first 5,000 results
      fetch: async ({ offset, n }) => {
        let data;
        try {
          data = await fetchView(view, offset, n);
        } catch (e) {
          if (view !== "COMPLETE" || (e.status !== 401 && e.status !== 403)) throw e;
          view = "STANDARD"; // no institutional entitlement: first author only, no abstract
          data = await fetchView(view, offset, n);
        }
        const sr = data["search-results"] || {};
        return { records: (sr.entry || []).filter((e) => !e.error).map(scopusRecord), total: parseInt(sr["opensearch:totalResults"] || "0", 10) };
      },
    });
    return out(c, query);
  });
  const scopusRecord = (e) =>
      R.make("scopus", {
        ids: { scopus: e.eid },
        title: e["dc:title"],
  ...
  attach("sciencedirect", async (o) => {
    ...
    const body = { qs, display: { offset: 0, show: 100, sortBy: "relevance" } };
    ...
    const c = await collect(o, {
      size: 100,
      max: 6000, // the ScienceDirect Search API hands out the first 6,000 results
      fetch: async ({ offset, n }) => {
        const res = await ZR.http("PUT", "https://api.elsevier.com/content/search/sciencedirect", { headers: elsevierHeaders(o), body: Object.assign({}, body, { display: { offset, show: n, sortBy: "relevance" } }), noRetry: true });
        const data = res.json();
        return { records: (data.results || []).map(sdRecord), total: data.resultsFound || 0 };
      },
    });
    return out(c, JSON.stringify({ qs: body.qs, date: body.date, filters: body.filters }));
  });
  const sdRecord = (r) =>
